<template>
  <div class="profile-page">
    ...
    <div class="user-header">
      <div class="avatar-container" @click="handleAvatarClick" :style="{ cursor: 'pointer' }">
        <img
      ...
      <div class="username-container">
        ...
        <van-icon name="edit" size="16" class="edit-icon" @click="showUsernameDialog = true" />
      </div>
    </div>
    ...
    <div class="section-container">
      ...
      <div class="books-grid">
        ...
        >
          <van-image :src="book.image_url" class="book-cover" fit="cover" />
      ...
      <div class="books-grid">
        ...
        >
          ...
          >
            ...
            <div v-if="r.remark" style="font-size: 12px; color: #444; margin-top: 6px">
              备注：{{ r.remark }}
            </div>
          </div>
        </div>
    ...
    <!-- 退出登录按钮 -->
    <van-button class="logout-btn" round @click="handleLogout"> 退出登录 </van-button>
    ...
    >
      <van-field v-model="newUsername" placeholder="请输入新用户名" maxlength="12" />
    </van-dialog>
    ...
    for (const u of urls) {
      try {
        ...
        for (const item of list) {
          ...
          const commentSuffix = commentContent
            ? ` ${commentContent}`
            : item.comment
              ? `#${item.comment}`
              : ''
  ...
  uploadImage(file)
    ...
    .then((userResponse) => {
      ...
      localStorage.setItem(
        AVATAR_CACHE_KEY,
        JSON.stringify({ time: Date.now(), avatarUrl: userResponse.data.profile_picture })
      )
...
const handleUsernameChange = async () => {
  ...
  try {
    const response = await updateUserInfo(userInfo.value.id, { username: trimmedName })
    ...
    showToast({
      message: '用户名更新失败：' + (err.response?.data?.message || err.message),
      duration: 1000
    })
  }
}
