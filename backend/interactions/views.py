...
from .models import Comment, CommentLike, Favorite, FavoriteFolder, Rating
from .serializers import CommentSerializer, FavoriteFolderSerializer, FavoriteSerializer, RatingSerializer
...
class FavoriteFolderViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        return Response(
            {"code": 201, "message": "收藏夹创建成功", "data": self.get_serializer(folder).data},
            status=status.HTTP_201_CREATED,
        )

    @swagger_auto_schema(
        operation_summary="删除收藏夹", responses={204: openapi.Response(description="收藏夹删除成功，作品转为未分组")}
    )
...
class FavoriteViewSet(viewsets.ModelViewSet):
    ...
    def list(self, request, *args, **kwargs):
        ...
        data = {"code": 200, "message": "获取收藏列表成功", "data": serializer.data}
    ...
    def create(self, request, *args, **kwargs):
        ...
        return Response(
            {"code": 201, "message": "收藏成功", "data": self.get_serializer(favorite).data},
            status=status.HTTP_201_CREATED,
        )
    ...
    @swagger_auto_schema(
        ...
        manual_parameters=[
            openapi.Parameter(
                name="id", in_=openapi.IN_PATH, type=openapi.TYPE_INTEGER, description="当前作品 ID", required=True
            )
        ],
    ...
    def destroy(self, request, *args, **kwargs):
        ...
        return Response({"code": 200, "message": "取消收藏成功"}, status=status.HTTP_200_OK)
    ...
    def move_to_folder(self, request):
        ...
        return Response({"code": 200, "message": "批量移动收藏成功", "data": serializer.data})
...
class CommentViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        queryset = Comment.objects.select_related("user", "gamework").annotate(like_count=Count("likes", distinct=True))

        if user.is_authenticated:
            queryset = queryset.prefetch_related(
                Prefetch("likes", queryset=CommentLike.objects.filter(user=user), to_attr="user_likes")
            )
    ...
    def list(self, request, *args, **kwargs):
        ...
        data = {"code": 200, "message": "获取评论列表成功", "data": serializer.data}
        return self.get_paginated_response(data) if page else Response(data)

    @swagger_auto_schema(
        operation_summary="发表评论或回复", request_body=CommentSerializer, responses={200: CommentSerializer}
    )
    def create(self, request, *args, **kwargs):
        serializer = self.get_serializer(data=request.data)
        serializer.is_valid(raise_exception=True)
        serializer.save()
        return Response(
            {"code": 201, "message": "评论发布成功", "data": serializer.data}, status=status.HTTP_201_CREATED
        )

    @swagger_auto_schema(
        operation_summary="删除评论（用户只能删除自己的评论，作者可删除本书评论。管理员可删除所有评论）",
        responses={200: "删除成功", 400: "参数错误", 403: "无权限删除该评论", 404: "评论不存在"},
    )
    def destroy(self, request, *args, **kwargs):
        comment_id = kwargs.get("pk")

        if not comment_id:
            return Response({"code": 400, "message": "缺少参数 id"}, status=400)
        ...
        if not user.is_staff and comment.user != user and comment.gamework.author != user:
            return Response({"code": 403, "message": "您没有权限删除该评论"}, status=403)
        ...
        return Response(
            {
                ...
                "data": {"liked": True, "like_count": comment.likes.count()},
            }
        )
    ...
    def unlike(self, request, pk=None):
        ...
        return Response(
            {"code": 200, "message": "取消点赞成功", "data": {"liked": False, "like_count": comment.likes.count()}}
        )
