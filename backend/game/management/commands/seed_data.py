    ...
    def handle(self, *args, **options):
        ...
        for i in range(game_count):
            ...
            story = Story.objects.create(
                gamework=gw, total_chapters=5, initial_generation_complete=True, is_complete=True, ai_callable=False
            )
            ...
            for c in range(1, 6):
                ...
                for s_idx in range(1, 4):  # 每个章节 3 个场景
                    ...
                    for d_idx in range(1, 16):  # 每个场景 15 段对话
                        dialogues.append(
                            {
                                "narration": f"这是第 {c} 章第 {s_idx} 场景的第 {d_idx} 段旁白文本。这里包含一些测试用的长文本内容，用于模拟真实的游戏剧情体验。"
                                * 2
                            }
                        )
                ...
                StoryChapter.objects.create(
                    ...
                    parsed_content={"title": f"第 {c} 章", "scenes": scenes},
                )
