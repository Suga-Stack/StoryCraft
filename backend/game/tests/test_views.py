...
class GameViewsTestCase(APITestCase):
    ...
    def setUp(self):
        ...
        self.story = Story.objects.create(
            gamework=self.gamework, total_chapters=3, ai_callable=True, initial_generation_complete=True
        )
        self.chapter = StoryChapter.objects.create(
            story=self.story, chapter_index=1, title="Chapter 1", status=StoryChapter.ChapterStatus.GENERATED
        )
        self.scene = StoryScene.objects.create(
            chapter=self.chapter, scene_index=1, background_image_url="https://example.com/image.png"
        )
    ...
    def test_game_create_view_success(self, mock_create_gamework):
        ...
        data = {"tags": ["奇幻", "玄幻", "仙侠"], "idea": "A grand adventure", "length": "medium", "modifiable": True}
    ...
    def test_game_save_put_create(self):
        ...
        data = {
            "title": "My Save",
            "timestamp": 123456789,
            "state": {"chapterIndex": 1, "sceneId": 1, "dialogueIndex": 1},
        }
        response = self.client.put(url, data, format="json")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_game_save_list_view(self):
        ...
        GameSave.objects.create(
            ...
            slot=2,
            title="Save 2",
            state={"chapterIndex": 2, "sceneId": 1},
            cover_url="url2",
            timestamp=2,
        )
    ...
    def test_game_ending_manual_update(self):
        ...
        url = reverse("story-ending-detail", kwargs={"gameworkId": self.gamework.id, "endingIndex": 1})
        data = {"endingIndex": 1, "title": "New Ending Title", "scenes": []}
        response = self.client.put(url, data, format="json")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_ending_generate_view(self, mock_start_gen):
        ...
        data = {"title": "Ending Title", "outline": "Ending Outline", "userPrompt": "Prompt"}
        response = self.client.post(url, data, format="json")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
