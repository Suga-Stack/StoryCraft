...
### 技术实现约束
- 每章字数目标：2000字
- 特别注意：第{total_chapters}章（最后一章）必须设计为多结局模式。请在章节大纲中明确指出“本章将根据玩家属性达成不同结局”，并简要列举可能的结局方向（如：完美结局、普通结局、遗憾结局）。
...
2. 选择不会影响最后一章结局之前的主线剧情，仅在最后一章才会根据属性值判定多结局，确保选择后能在3句话内回归主线
3. 选择点必须出现在自然的对话或决策情境后，选项要体现角色性格和处境
4. 选择后的反应：2-3句话展示立即后果，然后自然回归主线
5. 确保无论选择哪个选项，主线情节都能自然推进
6. 结局之前的主线剧情始终是线性的，不会因为玩家选择而影响主线剧情，玩家选择仅会带来后面几句话的即时响应。且必须能够自然过渡到后续主线情节
...
注意：确保选择点自然融入剧情，不要让玩家感觉选项是突兀插入的。
不要打破剧情与玩家的第四面窗，即主线剧情中不要出现“无论你怎么选择，xxx都会xxx”或跟“玩家选项”有关的话语。
"""
...
### 结局铺垫要求
- **氛围营造**：营造紧张、期待的氛围，让玩家感受到即将到来的结局
- **情感累积**：累积情感张力，为结局做情感准备
- **悬念设置**：设置适当的悬念，但不揭示答案

### 选择点嵌入规范（如果有选择点）
选择点必须以下列格式嵌入在剧情中（选择点必须出现在自然的对话或决策情境后）：
这里是前文剧情，引出玩家选择...
→ A. [选项描述] [影响属性：属性名±值]
...
### 停止时机
- 当剧情发展到**结局触发点**时立即停止
- 结局触发点示例：重大决策时刻、真相揭示前、最终对决前等
- 不要描述任何结局结果，只需将玩家带到结局的门槛
...
注意：
- 确保前半部分在合适的悬念点停止，为多结局留下空间
- 不要打破剧情与玩家的第四面墙，即主线剧情中不要出现"无论你怎么选择，xxx都会xxx"或跟"玩家选项"有关的话语
- 结局概述要基于整个游戏内容，而不仅仅是最后一章
- 确保所有结局都能自然地从前半部分的停止点展开
"""
