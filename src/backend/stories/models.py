...
class Story(models.Model):
    ...
    gamework = models.OneToOneField(
        Gamework, on_delete=models.CASCADE, related_name="story", help_text="关联的游戏作品"
    )
    ...
    # 生成状态
    initial_generation_complete = models.BooleanField(
        default=False, help_text="初始信息（标题、简介、封面、大纲）是否生成完成"
    )
    ...
    def update_generation_status(self):
        """更新生成状态"""
        self.initial_generation_complete = bool(self.core_seed and self.outlines and self.gamework.image_url)
        self.save()
...
class StoryChapter(models.Model):
    ...
    status = models.CharField(
        max_length=20, choices=ChapterStatus.choices, default=ChapterStatus.GENERATING, help_text="章节状态"
    )
    created_at = models.DateTimeField(auto_now_add=True)
...
class StoryEnding(models.Model):
    ...
    status = models.CharField(
        max_length=20, choices=EndingStatus.choices, default=EndingStatus.NOT_GENERATED, help_text="结局状态"
    )
...
class StoryScene(models.Model):
    ...
    chapter = models.ForeignKey(
        StoryChapter, on_delete=models.CASCADE, related_name="scenes", null=True, blank=True, help_text="关联的章节"
    )
    ending = models.ForeignKey(
        StoryEnding, on_delete=models.CASCADE, related_name="scenes", null=True, blank=True, help_text="关联的结局"
    )
    ...
    def __str__(self):
        if self.chapter:
            return (
                f"{self.chapter.story.gamework.title} - Chapter {self.chapter.chapter_index} - Scene {self.scene_index}"
            )
        elif self.ending:
