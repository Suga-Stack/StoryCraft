...
from .models import CommentReport, CreditLog, GameworkReport, SignInLog, UserSignIn
from .serializers import (
    CommentReportSerializer,
    CreditLogSerializer,
    GameworkReportSerializer,
    LoginSerializer,
    LogoutSerializer,
    RegisterSerializer,
...
from .utils import change_user_credits, get_signin_reward
...
class SendEmailCodeView(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            return Response(
                {"code": 500, "message": f"邮件发送失败: {str(e)}"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )
                                ...
                                "user": openapi.Schema(
                                    ...
                                    properties={
                                        ...
                                        "profile_picture": openapi.Schema(
                                            type=openapi.TYPE_STRING, description="头像URL", nullable=True
                                        ),
                                        "user_credits": openapi.Schema(
                                            type=openapi.TYPE_INTEGER, description="用户积分", nullable=True
                                        ),
...
class ReadGameworkListView(APIView):
    ...
    def get(self, request):
        user = request.user
        read_records = (
            ReadRecord.objects.filter(user=user, is_visible=True).select_related("gamework").order_by("-read_at")
        )
    ...
    def post(self, request):
        ...
        # 获取或创建阅读记录
        obj, created = ReadRecord.objects.get_or_create(user=user, gamework=gamework, defaults={"is_visible": True})
        ...
        if not obj.has_paid and gamework.price > 0:
            ...
            if (user.user_credits or 0) < gamework.price:
                return Response(
                    {"code": 400, "message": "积分不足，无法阅读该作品"}, status=status.HTTP_400_BAD_REQUEST
                )

            # 扣积分
            change_user_credits(
                user=user, amount=-gamework.price, log_type="read_pay", remark=f"阅读作品《{gamework.title}》扣费"
            )
            obj.has_paid = True
    ...
    @swagger_auto_schema(
        ...
        manual_parameters=[
            openapi.Parameter(
                "gamework_ids_param",
                openapi.IN_QUERY,
                description="要删除的作品ID列表，不传则删除所有",
    ...
    def delete(self, request):
        ...
        if gamework_ids_param:
            ...
            try:
                ...
                return Response(
                    {"code": 400, "message": "gamework_ids 参数格式错误"}, status=status.HTTP_400_BAD_REQUEST
                )

            # 只隐藏阅读记录，不删除
            updated = ReadRecord.objects.filter(user=user, gamework_id__in=gamework_ids, is_visible=True).update(
                is_visible=False
            )
...
class RecentReadGameworksView(APIView):
    ...
    @swagger_auto_schema(
        ...
        responses={200: openapi.Response(description="返回最近两部用户读过的作品", schema=GameworkSimpleSerializer)},
    )
    def get(self, request):
        user = request.user
        read_records = (
            ReadRecord.objects.filter(user=user, is_visible=True)
...
class MyGameworkListView(APIView):
    ...
    @swagger_auto_schema(
        ...
        responses={200: openapi.Response(description="当前用户创作的作品列表", schema=GameworkSimpleSerializer)},
    )
    def get(self, request):
        user = request.user
...
class RecentMyGameworksView(APIView):
    ...
    @swagger_auto_schema(
        ...
        responses={200: openapi.Response(description="最近两部创作的作品", schema=GameworkSimpleSerializer)},
    )
    def get(self, request):
        user = request.user
...
class UserSignInView(APIView):
    ...
    def get(self, request):
        ...
        # 筛选当前年份的签到记录
        dates = list(user.signin_logs.filter(date__year=year).order_by("-date").values_list("date", flat=True))
    ...
    @swagger_auto_schema(
        ...
        responses={
            ...
            status.HTTP_401_UNAUTHORIZED: openapi.Response(
                ...
                schema=openapi.Schema(
                    type=openapi.TYPE_OBJECT,
                    properties={"detail": openapi.Schema(type=openapi.TYPE_STRING, description="错误信息")},
                ),
            ),
        },
    )
    def post(self, request):
        ...
        # 增加用户积分
        change_user_credits(
            user=user, amount=reward, log_type="reward", remark=f"签到奖励（连续第 {signin_record.continuous_days} 天）"
        )
...
class RechargeViewSet(viewsets.ViewSet):
    ...
    @swagger_auto_schema(
        ...
        request_body=openapi.Schema(
            type=openapi.TYPE_OBJECT,
            properties={"credits": openapi.Schema(type=openapi.TYPE_INTEGER, description="充值积分数量")},
            required=["credits"],
        ),
        responses={
            200: openapi.Response(
    ...
    def create(self, request):
        ...
        user = request.user
        after = change_user_credits(user=user, amount=credits, log_type="recharge", remark="用户充值")

        return Response({"code": 200, "message": "充值成功", "new_credits": after}, status=200)
...
class RewardViewSet(viewsets.ViewSet):
    ...
    def create(self, request, pk=None):
        ...
        # 创建支出记录（打赏者）
        change_user_credits(
            user=user, amount=-amount, log_type="reward_out", remark=f"打赏作品《{gamework.title}》", gamework=gamework
        )

        author.refresh_from_db()
        # 创建收入记录（作者）
        change_user_credits(
            user=author, amount=amount, log_type="reward_in", remark=f"收到作品《{gamework.title}》的打赏"
        )

        return Response({"code": 200, "message": "打赏成功", "amount": amount, "author": author.username})
...
class CreditLogViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    @swagger_auto_schema(
        ...
        responses={200: openapi.Response(description="积分流水列表", schema=CreditLogSerializer(many=True))},
    )
    def list(self, request, *args, **kwargs):
...
class GameworkReportViewSet(viewsets.ModelViewSet):
    ...
    @swagger_auto_schema(
        operation_summary="举报作品", request_body=GameworkReportSerializer, responses={201: "举报成功"}
    )
    def create(self, request, *args, **kwargs):
        ...
        return Response({"code": 201, "message": "作品举报成功", "data": serializer.data}, status=201)
    ...
    def destroy(self, request, *args, **kwargs):
        if not request.user.is_staff:
            return Response({"code": 403, "message": "只有管理员可以删除举报记录"}, status=403)

        return super().destroy(request, *args, **kwargs)

    @swagger_auto_schema(
        operation_summary="处理作品举报（仅管理员）", request_body=no_body, responses={200: "处理完成", 403: "无权限"}
    )
    @action(detail=True, methods=["patch"], url_path="resolve")
    def resolve(self, request, *args, **kwargs):
        if not request.user.is_staff:
            return Response({"code": 403, "message": "只有管理员可以处理举报"}, status=403)

        report = self.get_object()
        report.is_resolved = True
        report.save()

        return Response({"code": 200, "message": "作品举报已标记为已处理"})
...
class CommentReportViewSet(viewsets.ModelViewSet):
    ...
    @swagger_auto_schema(
        operation_summary="举报评论", request_body=CommentReportSerializer, responses={201: "举报成功"}
    )
    def create(self, request, *args, **kwargs):
        serializer = self.get_serializer(data=request.data)
        serializer.is_valid(raise_exception=True)
        ...
        return Response({"code": 201, "message": "评论举报成功", "data": serializer.data}, status=201)
    ...
    @swagger_auto_schema(
        operation_summary="处理评论举报（仅管理员）", request_body=no_body, responses={200: "处理完成", 403: "无权限"}
    )
    @action(detail=True, methods=["patch"], url_path="resolve")
    def resolve(self, request, *args, **kwargs):
        if not request.user.is_staff:
        ...
        return Response({"code": 200, "message": "评论举报已标记为已处理"})
...
class SaveDetailView(APIView):
    ...
    def put(self, request, userId: int, workId: int, slot: str):
        ...
        obj, _ = GameSave.objects.update_or_create(
            user=target_user, gamework=gamework, name=slot, defaults={"game_state": data, "thumbnail": thumbnail}
        )
        return Response({"ok": True}, status=status.HTTP_200_OK)

    @swagger_auto_schema(
        operation_summary="读取存档",
        responses={200: openapi.Response(description='{"data": {...}}'), 404: openapi.Response(description="未找到")},
    )
    ...
    @swagger_auto_schema(operation_summary="删除存档", responses={200: openapi.Response(description='{"ok": true}')})
...
class SaveListView(APIView):
    ...
    @swagger_auto_schema(
        operation_summary="列出作品下全部存档摘要", responses={200: openapi.Response(description='{"saves": [...]}')}
    )
