...
class FavoriteViewSetTests(APITestCase):
    ...
    def test_create_duplicate_favorite(self):
        Favorite.objects.create(user=self.user, gamework=self.gamework)
        response = self.client.post("/api/interactions/favorites/", {"gamework_id": self.gamework.id})
    ...
    def test_update_move_to_folder(self):
        ...
        response = self.client.post(
            "/api/interactions/favorites/move_to_folder/",
    ...
    def test_move_to_folder(self):
        ...
        response = self.client.post(
            "/api/interactions/favorites/move_to_folder/",
            {"favorite_ids": [favorite1.id, favorite2.id], "folder": self.folder.id},
            format="json",
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_move_to_folder_invalid_ids_format(self):
        response = self.client.post(
            "/api/interactions/favorites/move_to_folder/",
            {"favorite_ids": ["invalid"], "folder": self.folder.id},
            format="json",
        )
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
...
class CommentViewSetTests(APITestCase):
    ...
    def test_create_comment(self):
        response = self.client.post(
            "/api/interactions/comments/", {"gamework": self.gamework.id, "content": "New Comment"}
        )
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
...
class RatingViewSetTests(APITestCase):
    ...
    def test_invalid_rating_score(self):
        response = self.client.post("/api/interactions/ratings/", {"id": self.gamework.id, "score": 15})
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
