...
from .game_generator.architecture import generate_architecture, generate_core_seeds
from .game_generator.chapter import (
    calculate_attributes,
    generate_chapter_content,
    generate_ending_content,
    parse_ending_condition,
)
...
def _select_background_music(tags: List[str]) -> List[Music]:
    ...
    # 查找有重叠标签的音乐，按重叠数量降序
    matches = (
        Music.objects.filter(tags__name__in=tags)
        .annotate(match_count=Count("tags", filter=Q(tags__name__in=tags)))
...
def _generate_chapter(gamework_id: int, chapter_index: int, user_prompt: str = ""):
    ...
    with transaction.atomic():
        ...
        for scene in chapter_dict["scenes"]:
            ...
            img_url = (
                scene_urls[idx]
                if idx < len(scene_urls)
                else f"{settings.SITE_DOMAIN}{settings.MEDIA_URL}placeholders/scene.jpg"
            )
            StoryScene.objects.create(
                chapter=chapter_obj, scene_index=scene["id"], background_image_url=img_url, dialogues=scene["dialogues"]
            )
    ...
    try:
        ...
        with transaction.atomic():
            ...
            for scene in parsed_ending["scenes"]:
                ...
                StoryScene.objects.create(
                    ending=ending, scene_index=scene["id"], background_image_url=img_url, dialogues=scene["dialogues"]
                )
...
def _generate_and_save_report(story: Story, ending_obj: StoryEnding):
    ...
    try:
        ...
        report_data = generate_report_content(
            global_summary=story.global_summary, ending_summary=ending_obj.summary, ending_title=ending_obj.title
        )
...
def get_ending_report(gamework_id: int, ending_index: int, player_attributes: dict) -> dict:
    ...
    return {
        "status": "ready",
        "details": {"title": report.title, "content": report.content, "traits": report.traits, "scores": scores},
    }
...
def get_story_endings(gamework: Gamework) -> dict:
    ...
    return {"status": "ready", "endings": endings_data}
...
def _generate_all_chapters_async(gamework_id: int):
    ...
    def worker():
        try:
            ...
            # 获取总章节数
            total_chapters = (
                Story.objects.filter(gamework_id=gamework_id).values_list("total_chapters", flat=True).first()
            )
            ...
            Story.objects.filter(gamework_id=gamework_id).update(
                is_generating=False, is_complete=True, current_generating_chapter=0
            )
...
def resolve_scene_cover_url(
    gamework: Gamework, chapter_index: Optional[int], scene_index: int, ending_index: Optional[int] = None
) -> Optional[str]:
    """根据章节与场景索引解析背景图URL"""
    url = None
    ...
    if ending_index is not None:
        url = (
            StoryScene.objects.filter(
                ending__story__gamework=gamework, ending__ending_index=ending_index, scene_index=scene_index
            )
            .values_list("background_image_url", flat=True)
            .first()
        )
    elif chapter_index is not None:
        url = (
            StoryScene.objects.filter(
                chapter__story__gamework=gamework, chapter__chapter_index=chapter_index, scene_index=scene_index
            )
            .values_list("background_image_url", flat=True)
            .first()
        )
