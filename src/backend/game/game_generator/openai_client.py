...
def invoke(messages) -> str:
    ...
    for attempt in range(max_retries + 1):
        try:
            response = text_client.chat.completions.create(model=settings.AI_MODEL_FOR_TEXT, messages=messages)
...
def generate_single_image(prompt: str, size: str = "2560x1440") -> str:
    ...
    for attempt in range(max_retries + 1):
        try:
            response = image_client.images.generate(model=settings.AI_MODEL_FOR_IMAGE, prompt=prompt, size=size)
