...
class GameworkViewSetTestCase(APITestCase):
    ...
    def setUp(self):
        ...
        self.unpublished_gamework = Gamework.objects.create(
            author=self.user, title="Unpublished Gamework", is_published=False
        )
        self.other_user_gamework = Gamework.objects.create(
            author=self.other_user, title="Other User's Gamework", is_published=True
        )
        self.story = Story.objects.create(gamework=self.gamework, initial_generation_complete=True)
        self.unfinished_story = Story.objects.create(
            gamework=self.unpublished_gamework, initial_generation_complete=False
        )
        self.other_user_story = Story.objects.create(
            gamework=self.other_user_gamework, initial_generation_complete=True
        )
...
class PublishGameworkViewSetTestCase(APITestCase):
    ...
    def setUp(self):
        ...
        self.gamework = Gamework.objects.create(author=self.user, title="Test Gamework", is_published=False)
        self.other_user_gamework = Gamework.objects.create(
            author=self.other_user, title="Other User's Gamework", is_published=False
        )
...
class GameworkFavoriteLeaderboardViewSetTestCase(APITestCase):
    ...
    def setUp(self):
        ...
        self.gamework1 = Gamework.objects.create(author=self.user, title="Gamework 1", is_published=True)
        self.gamework2 = Gamework.objects.create(author=self.user, title="Gamework 2", is_published=True)
        self.gamework3 = Gamework.objects.create(author=self.user, title="Gamework 3", is_published=True)
...
class GameworkSearchViewTestCase(APITestCase):
    ...
    def setUp(self):
        ...
        self.gamework1 = Gamework.objects.create(
            author=self.user, title="冒险之旅", description="一个冒险的故事", is_published=True
        )
        self.gamework1.tags.add(self.tag_adventure, self.tag_rpg)

        self.gamework2 = Gamework.objects.create(
            author=self.user, title="战斗传奇", description="一个传奇的战斗故事", is_published=True
        )
        self.gamework2.tags.add(self.tag_adventure)

        self.gamework3 = Gamework.objects.create(
            author=self.user, title="魔法世界", description="探索魔法世界", is_published=True
        )
...
class RecommendViewTestCase(APITestCase):
    ...
    def setUp(self):
        ...
        # Create sample gameworks
        self.gamework1 = Gamework.objects.create(
            author=self.other_user, title="冒险之旅", description="一个冒险的故事", is_published=True
        )
        self.gamework1.tags.add(self.tag_adventure, self.tag_rpg)

        self.gamework2 = Gamework.objects.create(
            author=self.other_user, title="战斗传奇", description="一个传奇的战斗故事", is_published=True
        )
        self.gamework2.tags.add(self.tag_adventure)

        self.gamework3 = Gamework.objects.create(
            author=self.other_user, title="魔法世界", description="探索魔法世界", is_published=True
        )
        self.gamework3.tags.add(self.tag_rpg)
    ...
    def test_recommendations_exclude_user_created(self):
        ...
        self.gamework4 = Gamework.objects.create(
            author=self.user, title="用户自己的游戏", description="该作品是用户自己创作的", is_published=True
        )
