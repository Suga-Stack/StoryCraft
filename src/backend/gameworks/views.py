...
from django.db.models import Avg, Count, F, IntegerField, OuterRef, Prefetch, Q, Subquery
...
class GameworkViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if user.is_authenticated:
            queryset = queryset.prefetch_related(
                Prefetch("favorited_by", queryset=Favorite.objects.filter(user=user), to_attr="user_favorites")
            )
    ...
    def destroy(self, request, *args, **kwargs):
        ...
        if not user.is_staff and instance.author != user:
            return Response({"detail": "只有管理员或作者可以删除作品"}, status=403)
...
class PublishGameworkViewSet(viewsets.ViewSet):
    ...
    @swagger_auto_schema(
        ...
        operation_description=("作品发布前对作者和管理员以外不可见。\n\n"),
        request_body=openapi.Schema(
            type=openapi.TYPE_OBJECT,
            properties={"price": openapi.Schema(type=openapi.TYPE_INTEGER, description="作品价格（积分）")},
        ),
        responses={
    ...
    def publish(self, request, pk=None):
        ...
        if price is not None:
            ...
            if price < 0 or price > 50:
                return Response({"message": "price 必须在 0 ~ 50 之间"}, status=status.HTTP_400_BAD_REQUEST)
        ...
        return Response(
            {"message": "作品已成功发布", "gamework": GameworkSimpleSerializer(gamework).data},
...
class GameworkSearchView(generics.ListAPIView):
    ...
    @swagger_auto_schema(
        ...
        manual_parameters=[
            openapi.Parameter("q", openapi.IN_QUERY, description="关键词（标题或简介匹配）", type=openapi.TYPE_STRING),
            openapi.Parameter("author", openapi.IN_QUERY, description="作者用户名", type=openapi.TYPE_STRING),
            openapi.Parameter("tag", openapi.IN_QUERY, description="标签 ID 或标签名", type=openapi.TYPE_STRING),
        ],
        responses={
            200: openapi.Response(
                ...
                examples={
                    "application/json": {
                        "code": 200,
                        "message": "搜索成功",
                        "data": [{"id": 1, "title": "冒险之旅", "author": "Alice", "tags": ["冒险", "RPG"]}],
                    }
                },
            )
        },
    ...
    def get(self, request, *args, **kwargs):
        ...
        return Response({"code": 200, "message": "搜索成功", "data": serializer.data}, status=status.HTTP_200_OK)
...
class RecommendView(generics.ListAPIView):
    ...
    @swagger_auto_schema(
        ...
        responses={200: openapi.Response("推荐结果", GameworkSimpleSerializer(many=True)), 404: "用户未设置喜欢的标签"},
    )
    def get(self, request, *args, **kwargs):
        user = request.user
        ...
        if not liked_tags.exists():
            return Response(
                {"code": 404, "message": "用户未设置喜欢的标签", "data": []}, status=status.HTTP_404_NOT_FOUND
            )
        ...
        serializer = self.get_serializer(queryset, many=True)
        return Response({"code": 200, "message": "推荐成功", "data": serializer.data}, status=status.HTTP_200_OK)
...
class GameworkFavoriteLeaderboardViewSet(viewsets.ViewSet):
    ...
    @swagger_auto_schema(
        operation_summary="作品排行榜（收藏量）", operation_description=("返回收藏最多的10部作品，降序排列")
    )
    def list(self, request):
        ...
        return Response({"message": "作品排行榜", "data": serializer.data})
...
class GameworkRatingLeaderboardViewSet(viewsets.ViewSet):
    ...
    @swagger_auto_schema(
        operation_summary="作品排行榜（评分）", operation_description=("返回平均分最高的10部作品，降序排列")
    )
    def list(self, request):
        # 查询作品并按评分的平均值排序（降序）
        queryset = (
            Gamework.objects.filter(is_published=True)
            .annotate(average_score=Avg(F("ratings__score")))
            .order_by("-average_score")[:10]
        )

        # 序列化数据
        serializer = GameworkSimpleSerializer(queryset, many=True)
        return Response({"message": "作品评分排行榜", "data": serializer.data})
...
class GameworkHotLeaderboardViewSet(viewsets.ViewSet):
    ...
    def list(self, request):
        ...
        if time_range == "month":
            ...
            return Response(
                {"message": "无效的 range 参数，可选值: total, month, week"}, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        queryset = (
            ...
            .annotate(hot_score=F("reply_count") * 2 + F("favorite_count") * 1)
        ...
        return Response(
            {"message": f"作品热度排行榜（{time_range}）", "data": serializer.data}, status=status.HTTP_200_OK
        )
