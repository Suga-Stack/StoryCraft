  ...
  <div class="credits-charge-page">
    ...
    <main class="main-content">
      ...
      <div class="recharge-grid">
        <div
          class="recharge-card"
          v-for="(item, index) in fixedRechargeOptions"
          :key="index"
      ...
      <div v-if="showChargeLog" class="credits-log-container">
        ...
        <!-- 空状态 -->
        <div v-if="!loading && creditsLog.length === 0" class="empty-log">暂无积分记录</div>
        ...
        <div class="pagination" v-if="!loading && creditsLog.length > 0">
          ...
          <button class="page-btn" :disabled="!hasMore" @click="fetchCreditsLog(currentPage + 1)">
            下一页
          </button>
        </div>
      </div>
      ...
      <div class="model-overlay" v-show="showChargeModal" @click="showChargeModal = false">
        <transition name="scale-pop">
          <div
            ...
            @click.stop
            :key="showChargeModal ? 'open' : 'close'"
          >
            <!--弹窗导航栏-->
      ...
      <div
        class="model-overlay"
        v-show="showChargeSuccessModal"
        @click="showChargeSuccessModal = false"
      >
        <transition name="scale-pop">
          <div
            class="charge-success-model"
            v-if="showChargeSuccessModal"
            @click.stop
            :key="showChargeSuccessModal ? 'open' : 'close'"
          >
            <!--弹窗充值信息卡片-->
            <div class="charge-card">
              <!-- 心形图标 -->
