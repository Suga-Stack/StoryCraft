...
export function useCreatorMode(dependencies = {}) {
  ...
  const loadOverrides = (workId) => {
    try {
      try {
        localStorage.removeItem(overridesKey(userId, workId))
      } catch (e) {}
  ...
  const saveOverrides = (workId) => {
    try {
      ...
      try {
        ...
        console.warn('保存 overrides 失败', e)
      }
    }
  }
    ...
    try {
      ...
      for (const sid in overrides.value.scenes) {
        ...
        for (let i = 0; i < storyScenes.value.length; i++) {
          ...
          const key = String(
            (s && (s._uid ?? s.sceneId ?? s.id)) != null
              ? (s._uid ?? s.sceneId ?? s.id)
              : `idx_${i}`
          )
          if (key === String(sid)) {
            sIdx = i
        ...
        if (ov.dialogues) {
          for (const k in ov.dialogues) {
            ...
            if (
              !isNaN(idx) &&
              storyScenes.value[sIdx].dialogues &&
              idx < storyScenes.value[sIdx].dialogues.length
            ) {
      ...
      try {
        ...
        console.warn('force refresh after applyOverridesToScenes failed', e)
      }
    } catch (e) {
      console.warn('applyOverridesToScenes failed', e)
    }
  }
  ...
  const toggleCreatorMode = async (params = {}) => {
    try {
      ...
      const _creatorFeatureEnabled =
        params.creatorFeatureEnabled || dependencies.creatorFeatureEnabled
      ...
      const _persistCurrentChapterEdits =
        params.persistCurrentChapterEdits || dependencies.persistCurrentChapterEdits
      ...
      if (!creatorMode.value) {
        ...
        if (isManualEditOnly) {
          console.log(
            '[toggleCreatorMode] 检测到手动编辑模式 (modifiable=true, ai_callable=false)，跳过所有章节状态检查'
          )
        ...
        try {
          ...
          if (!isManualEditOnly && (isCreatorIdentity?.value || modifiableFromCreate?.value)) {
            ...
            if (
              cur &&
              (cur._isBackendEnding || cur.isGameEnding || cur.isEnding) &&
              cur._endingSaved !== true
            ) {
        ...
        if (_persistCurrentChapterEdits) {
          try {
            await _persistCurrentChapterEdits({ auto: true, performNetworkSave: false })
          } catch (e) {}
        }
      }
      creatorMode.value = !creatorMode.value
    } catch (e) {
      console.warn('toggleCreatorMode failed', e)
    }
  }
  ...
  const openOutlineEditorManual = async (params = {}) => {
    try {
      ...
      const start =
        Number(currentChapterIndex?.value || params.currentChapterIndex?.value || 1) || 1
      const total = Math.max(
        Number(totalChapters?.value || params.totalChapters?.value || 0) || 5,
        0
      )
      ...
      try {
        ...
        if (workId && typeof getWorkDetails === 'function') {
          ...
          console.log(
            '[openOutlineEditorManual] 已从后端加载大纲数据，共',
            rawOutlines.length,
            '章'
          )
        }
      ...
      if (Array.isArray(rawOutlines)) {
        for (let i = 0; i < rawOutlines.length; i++) {
          ...
          try {
            ...
            outlinesMap[ci] = { title: '', outline: JSON.stringify(ch) }
          }
      ...
      console.log(
        '[useCreatorMode] 打开大纲编辑器: reason=manual-invocation, targetChapter=',
        pendingOutlineTargetChapter.value
      )
      showOutlineEditor.value = true
    } catch (e) {
      console.warn('openOutlineEditorManual failed', e)
    }
  }
    ...
    ;(async () => {
      try {
        ...
        if (editorInvocation.value === 'auto' || creatorMode.value) {
          ...
          const payloadOutlines = (originalOutlineSnapshot.value || []).map((o) => ({
          ...
          try {
            ...
            if (generationLocks.value[lockKey]) {
              ...
              try {
                await generateChapter(workId, tChap, {
                  chapterOutlines: payloadOutlines,
                  userPrompt: outlineUserPrompt.value
                })
              } catch (e) {
                console.warn('cancelOutlineEdits generate failed', e)
              } finally {
                try {
                  delete generationLocks.value[lockKey]
                } catch (ee) {}
              }
            }
          } catch (e) {
            console.warn('cancelOutlineEdits generate flow failed', e)
          }
        }
      } catch (e) {
        console.warn('cancelOutlineEdits async failed', e)
      }
    })()

    if (typeof outlineEditorResolver === 'function') {
      outlineEditorResolver(false)
      outlineEditorResolver = null
    }
  ...
  const confirmOutlineEdits = async (params = {}) => {
    ...
    try {
      ...
      const targetChapter =
        pendingOutlineTargetChapter.value || outlineEdits.value?.[0]?.chapterIndex || 1

      // 组装章节大纲 payload（后端期望 summary 字段）
      // 注意：与后端 serializers.ChapterOutlineSerializer 保持一致: {chapterIndex, outline}
      let payloadOutlines = (outlineEdits.value || []).map((o) => ({
        chapterIndex: o.chapterIndex,
        outline: o.outline
      }))
      ...
      try {
        ...
        try {
          ...
          /* ignore start loading errors */
        }

        await generateChapter(workId, targetChapter, {
          chapterOutlines: payloadOutlines,
          userPrompt: outlineUserPrompt.value
        })
        ...
        // 状态就绪后拉取该章剧情
        try {
          await fetchNextChapter(workId, targetChapter, {
            replace: true,
            singleRequest: true,
            suppressAutoEditor: true
          })
        } catch (fetchErr) {
        ...
        try {
          if (typeof stopLoading === 'function') {
            ...
            _startedLoadingHere &&
            dependencies &&
            typeof dependencies.stopLoading === 'function'
          ) {
            try {
              await dependencies.stopLoading()
            } catch (e) {}
          }
        } catch (e) {}
      }
  ...
  const startEdit = async (params = {}) => {
    ...
    setTimeout(() => {
      try {
        ...
        if (el) {
          try {
            el.innerText = editableText.value
          } catch (e) {}
          el.focus()
          try {
  ...
  const onEditableInput = (e) => {
    try {
      ...
      console.warn('onEditableInput failed', err)
    }
  }
  ...
  const onCompositionStart = () => {
    try {
      ...
      console.warn('onCompositionStart failed', err)
    }
  }
  ...
  const onCompositionEnd = (e) => {
    try {
      ...
      console.warn('onCompositionEnd failed', err)
    }
  }
  ...
  const finishEdit = (params = {}) => {
    ...
    try {
      ...
      try {
        if (_previewSnapshot) _previewSnapshot.value = null
      } catch (e) {}
    } catch (e) {
      console.warn('finishEdit failed', e)
    }
  ...
  const isNarration = (item) => {
    try {
      ...
      if (typeof item === 'object') {
        ...
        if (
          !item.speaker &&
          item._fromChoiceId == null &&
          item._fromChoiceIndex == null &&
  ...
  const addNarration = async (params = {}) => {
    ...
    try {
      ...
      let insertIndex =
        _currentDialogueIndex?.value != null
          ? _currentDialogueIndex.value + 1
          : scene.dialogues.length
      if (insertIndex < 0 || insertIndex > scene.dialogues.length)
        insertIndex = scene.dialogues.length
      ...
      const existingKeys = Object.keys(od)
        .map((k) => Number(k))
        .filter((k) => !isNaN(k))
        .sort((a, b) => b - a)
      ...
      // 将当前编辑索引跳到新旁白
      try {
        _currentDialogueIndex.value = insertIndex
      } catch (e) {}
      showToast?.('已插入旁白')
    } catch (e) {
  ...
  const deleteNarration = (params = {}) => {
    ...
    try {
      ...
      if (!Array.isArray(scene.dialogues) || idx < 0 || idx >= scene.dialogues.length) {
        showToast?.('当前对话索引无效')
        return
      }
      const target = scene.dialogues[idx]
      if (!isNarration(target)) {
        showToast?.('当前项不是旁白，无法删除')
        return
      }
      ...
      const triggerIdx =
        typeof scene.choiceTriggerIndex === 'number' ? scene.choiceTriggerIndex : null
      ...
      if (hasChoices && oldTriggerIdx !== null && idx < oldTriggerIdx) {
        try {
          scene.choiceTriggerIndex = oldTriggerIdx - 1
        } catch (e) {
          /* ignore */
        }
      }
      ...
      for (let i = 0; i < scene.dialogues.length; i++) {
        ...
        if (typeof item === 'string') txt = item
          ...
          txt = item.text || item.narration || item.content || ''
    ...
    try {
      ...
      reader.onload = async () => {
        ...
        try {
          ...
          form.append('file', f)
          try {
            const resp = await http.post('/api/game/upload-image/', form, {
              headers: { 'Content-Type': 'multipart/form-data' }
            })
            const imageUrl =
              (resp && resp.data && (resp.data.imageUrl || resp.data.imageUrl)) ||
              (resp && resp.imageUrl) ||
              null
            if (imageUrl) {
      ...
      console.warn('onImageSelected failed', e)
    }
  }
  ...
  const playNextAfterEdit = (params = {}) => {
    ...
    try {
      ...
      try {
        if (_showMenu) _showMenu.value = false
      } catch (e) {}
      setTimeout(() => {
        if (_nextDialogue) {
      ...
      console.warn('playNextAfterEdit failed', e)
    }
  }
        ...
        try {
          ...
          try {
            if (pendingNextChapter.value != null) {
              ...
              pendingNextChapter.value = null(async () => {
                try {
                  startLoading()
  ...
  const setDependencies = (deps) => {
    ...
    if (deps.persistCurrentChapterEdits)
      dependencies.persistCurrentChapterEdits = deps.persistCurrentChapterEdits
