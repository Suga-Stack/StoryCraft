...
export function useSaveLoad() {
  ...
  const isCurrentBackendEndingSaved = () => {
    try {
      ...
      if (
        _currentScene &&
        _currentScene.value &&
        (_currentScene.value._isBackendEnding || _currentScene.value.isEnding)
      ) {
      ...
      if (
        _lastSelectedEndingIndex &&
        _lastSelectedEndingIndex.value &&
        _storyScenes &&
        Array.isArray(_storyScenes.value)
      ) {
  ...
  const saveGame = async (slot = 'default') => {
    try {
      ...
      try {
        ...
        console.warn('saveGame: chapter status check failed', e)
      }
      ...
      const gameData = {
        ...
        // 仍保留缩略图以便 UI/后端使用
        thumbnail:
          _currentBackground && _currentBackground.value
            ? _currentBackground.value
            : _effectiveCoverUrl && _effectiveCoverUrl.value
              ? _effectiveCoverUrl.value
              : work.value && work.value.coverUrl
                ? work.value.coverUrl
                : null
      }
      ...
      console.log(
        'saveGame: preparing to save. save type:',
        gameData.state && gameData.state.endingindex ? 'ENDING' : 'CHAPTER'
      )
  ...
  const buildSavePayload = () => {
    ...
    const cleanedChoiceHistory = (
      _choiceHistory && _choiceHistory.value ? _choiceHistory.value : []
    ).map((choice) => {
      ...
      return {
        chapterIndex:
          choice.chapterIndex || (_currentChapterIndex ? _currentChapterIndex.value : 1),
    ...
    const state = {
      sceneId:
        _currentScene &&
        _currentScene.value &&
        (_currentScene.value.id || _currentScene.value.sceneId)
    ...
    try {
      ...
      try {
        if (_storyScenes && Array.isArray(_storyScenes.value)) {
          for (const s of _storyScenes.value) {
            if (!s) continue
            if (s._isEndingChoiceScene || s._isBackendEnding) {
              hasBackendEndings = true
              break
            }
            if (Array.isArray(s.choices)) {
              for (const c of s.choices) {
                if (!c) continue
                if (Array.isArray(c._endingScenes) && c._endingScenes.length > 0) {
                  hasBackendEndings = true
                  break
                }
              }
        ...
        console.warn('buildSavePayload: scanning storyScenes for backend endings failed', scanErr)
      }
      ...
      if (hasBackendEndings) {
        ...
        if (_lastSelectedEndingIndex && _lastSelectedEndingIndex.value)
          recordedIdx = Number(_lastSelectedEndingIndex.value)
        ...
        try {
          if (_currentScene && _currentScene.value && Array.isArray(_currentScene.value.choices)) {
            ...
            if (chosenId != null) {
              ...
              const found = choicesArr.findIndex(
                (c) => c && (String(c.id) === String(chosenId) || c.choiceId == chosenId)
              )
              if (found >= 0) {
                const choiceObj = choicesArr[found]
                if (choiceObj && choiceObj._endingIndex != null)
                  currentChoiceIdx = Number(choiceObj._endingIndex)
                else currentChoiceIdx = found + 1
              }
            }
          }
        } catch (e) {
          console.warn('buildSavePayload: derive currentChoiceIdx failed', e)
        }

        let idx =
          currentChoiceIdx != null ? currentChoiceIdx : recordedIdx != null ? recordedIdx : null
        if (!idx || isNaN(idx)) idx = 1
        // 为了兼容不同命名约定，保存时同时写入小写和驼峰两种字段
        state.endingindex = idx
        state.endingIndex = idx
        console.log(
          'buildSavePayload: detected BACKEND endings — creating ENDING save with endingindex=',
          state.endingindex
        )
        // 尝试从结局场景中选取缩略图
        try {
          let endingThumb = null
          for (const s of _storyScenes && Array.isArray(_storyScenes.value)
            ? _storyScenes.value
            : []) {
            if (!s) continue
            if (s._isBackendEnding || s._isEndingChoiceScene) {
              if (s.backgroundImage) {
                endingThumb = s.backgroundImage
                break
              }
              if (Array.isArray(s.dialogues) && s.dialogues.length > 0) {
                const first = s.dialogues[0]
                if (first && (first.backgroundImage || first.bg)) {
                  endingThumb = first.backgroundImage || first.bg
                  break
                }
              }
            }
          ...
          console.warn('buildSavePayload: selecting ending thumbnail failed', thumbErr)
        }
      } else {
        state.chapterIndex = _currentChapterIndex ? _currentChapterIndex.value : 1
        console.log(
          'buildSavePayload: no backend endings detected — creating NORMAL chapter save for chapterIndex=',
          state.chapterIndex
        )
      }
    } catch (e) {
  ...
  const autoSaveToSlot = async (slot = AUTO_SAVE_SLOT) => {
    ...
    try {
      ...
      try {
        ...
        console.warn('autoSaveToSlot: chapter status check failed', e)
      }
      // 构建 gameData 对象
      const gameData = {
        work: work.value,
      ...
      // 使用 saveLoad.js 中的统一存档函数
      console.log(
        'autoSaveToSlot: preparing to auto-save. save type:',
        gameData.state && gameData.state.endingindex ? 'ENDING' : 'CHAPTER'
      )
      const result = await saveGameData(gameData, slot)
  ...
  const quickLocalAutoSave = (slot = AUTO_SAVE_SLOT) => {
    try {
      ...
      try {
        ...
        console.warn('quickLocalAutoSave: chapter status check failed', e)
      }
      ...
      console.log(
        'quickLocalAutoSave: saving locally. save type:',
        payload && payload.state && payload.state.endingindex ? 'ENDING' : 'CHAPTER'
      )
  ...
  const loadGame = async (slot = 'default') => {
    try {
      ...
      const savedEndingIndex =
        typeof remote.endingindex === 'number'
          ? remote.endingindex
          : remote.endingIndex != null
            ? Number(remote.endingIndex)
            : null
      ...
      try {
        ...
        if (savedEndingIndex) {
          ...
          try {
            ...
            if (
              payload &&
              (payload.status === 'generating' || payload.status === 'not_generated')
            ) {
              console.warn(`⚠️ 结局 ${savedEndingIndex} 尚未生成 (status=${payload.status})`)
              // 若未生成或正在生成，仍尝试从 payload.ending 中读取 scenes（若有）
            }
            const endingObj =
              payload && (payload.ending || payload) ? payload.ending || payload : null
            // 如果结局详情中包含 state，则该 state 应视为结局的官方保存状态，覆盖当前 remote
            try {
              const endingState =
                payload && (payload.state || (payload.ending && payload.ending.state))
                  ? payload.state || payload.ending.state
                  : null
              if (endingState) {
                remote = endingState
              ...
              console.warn('loadGame: 合并结局 state 失败', e)
            }
            ...
            const scenes = Array.isArray(endingObj?.scenes)
              ? endingObj.scenes
            ...
            if (!scenes || scenes.length === 0) {
              ...
              const list = Array.isArray(listPayload)
                ? listPayload
                : Array.isArray(listPayload?.endings)
                  ? listPayload.endings
                  : []
              ...
              if (chosen && Array.isArray(chosen.scenes)) {
                for (const scene of chosen.scenes) {
                  try {
                    ...
                    try {
                      const lastIdx =
                        _storyScenes && Array.isArray(_storyScenes.value)
                          ? _storyScenes.value.length - 1
                          : null
                      const pushed =
                        lastIdx != null && lastIdx >= 0 ? _storyScenes.value[lastIdx] : null
                      if (pushed) {
                      ...
                      console.warn('tagging pushed ending scene failed (fallback)', tagErr)
                    }
                  } catch (e) {
                    console.warn(
                      'pushSceneFromServer failed when loading ending scene (fallback):',
                      e
                    )
                  }
                }
              ...
              for (const scene of scenes) {
                try {
                  ...
                  console.warn('pushSceneFromServer failed when loading ending scene:', e)
                }
              }
            }
            ...
            try {
                ...
                for (let i = 0; i < _storyScenes.value.length; i++) {
                  try {
                    ...
                    if (pushed) {
                      ...
                      pushed.isEnding = true
                      pushed.endingIndex = savedEndingIndex
                      if (typeof pushed._endingSaved === 'undefined')
                        pushed._endingSaved = endingSavedFlag
                    }
                  } catch (e) {}
                }
              ...
              console.warn('marking loaded ending scenes failed', tagErr)
            }
            ...
            try {
              if (_lastSelectedEndingIndex) {
                ...
                console.log(
                  'loadGame: set _lastSelectedEndingIndex =',
                  _lastSelectedEndingIndex.value
                )
              }
            } catch (e) {
              console.warn('loadGame: set lastSelectedEndingIndex failed', e)
            }
            ...
            try {
              ...
              console.warn('loadGame: set playingEndingScenes failed', e)
            }
            ...
            try {
              ...
              console.warn('loadGame: set endingsAppended failed', e)
            }
          } catch (err) {
          ...
          const chapterData = _fetchNextChapter
            ? await _fetchNextChapter(workId, chapterToLoad)
            : null
          if (chapterData && chapterData.chapter && Array.isArray(chapterData.chapter.scenes)) {
            console.log(
              `✅ 成功获取章节 ${chapterToLoad} 的内容，共 ${chapterData.chapter.scenes.length} 个场景`
            )
      ...
      const deriveIndexFromPayload = (p) => {
        try {
          ...
          if (p.sceneId != null && _storyScenes && Array.isArray(_storyScenes.value)) {
            ...
            const idx = _storyScenes.value.findIndex(
              (s) => s && (String(s.id) === pid || String(s.sceneId) === pid)
            )
            if (idx >= 0) return idx
          ...
          if (typeof p.chapterIndex === 'number' && _storyScenes) {
            const idx = _storyScenes.value.findIndex(
              (s) => s && (s.chapterIndex === p.chapterIndex || s.chapter === p.chapterIndex)
            )
            if (idx >= 0) return idx
          }
        } catch (e) {}
      ...
      // 恢复显示状态
      try {
        if (_suppressAutoShowChoices) _suppressAutoShowChoices.value = false
      } catch (e) {}
      ...
      if (_currentChapterIndex && _currentSceneIndex && _currentDialogueIndex) {
        console.log(
          `📍 当前位置: 章节${_currentChapterIndex.value}, 场景${_currentSceneIndex.value}, 对话${_currentDialogueIndex.value}`
        )
      }
